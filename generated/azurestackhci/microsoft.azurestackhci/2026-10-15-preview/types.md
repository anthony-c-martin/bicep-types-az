...
* **kind**: string: This property identifies the purpose of the Cluster deployment. For example, a valid value is AzureLocal
...
* **sdnPrefix**: string: A string identifier used to construct the Network Controller (NC) REST resource name. This prefix helps group and distinguish SDN-managed network components and must follow specific formatting rules.
...
* **percentComplete**: int (ReadOnly): The progress percentage of the step.
...
### Properties
* **prefix**: string (Required): Naming prefix used to generate local availability zone names (Prefix1..PrefixN).
* **zones**: [LocalAvailabilityZoneEntry](#localavailabilityzoneentry)[] (Required): Array of zone-to-node mappings. Each entry specifies the nodes belonging to one local availability zone. Maximum 8 zones allowed. Each zone must contain at least one node. Every cluster node must be assigned to exactly one zone.
...
* **subnetMask**: string: Subnet mask.
* **switchName**: string (ReadOnly): The switch attached to the NIC, if any.
* **vlanId**: int (ReadOnly): VLAN identifier (0-4096).
...
* **ownershipVoucher**: string (Required): Ownership voucher in base64 encoded format
...
* **port**: string: Port of the web proxy.
