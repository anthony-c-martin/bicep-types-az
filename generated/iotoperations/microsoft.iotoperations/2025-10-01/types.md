...
* **keepAliveSeconds**: int: KeepAlive for connection in seconds.
* **maxInflightMessages**: int: The max number of messages to keep in flight. For subscribe, this is the receive maximum. For publish, this is the maximum number of messages to send before waiting for an ack.
* **protocol**: 'Mqtt' | string: The protocol to use for the connection. Currently only `mqtt` is supported.
* **sessionExpirySeconds**: int: Session expiry in seconds.
...
#### Properties
* **bucketSize**: int (Required): The bucketized allocation of AEPs for connectors.
...
* **endpointType**: string (Required): The type of the device inbound endpoint.
* **version**: string: The version of the device inbound endpoint.
...
* **deviceInboundEndpointTypes**: [AkriConnectorTemplateDeviceInboundEndpointType](#akriconnectortemplatedeviceinboundendpointtype)[] (Required): Device inbound endpoint types.
...
## BackendChain
### Properties
* **partitions**: int (Required): The desired number of physical backend partitions.
* **redundancyFactor**: int (Required): The desired numbers of backend replicas (pods) in a physical partition.
* **workers**: int: Number of logical backend workers per replica (pod).

## BatchingConfiguration
### Properties
* **latencySeconds**: int: Batching latency in seconds.
* **maxMessages**: int: Maximum number of messages in a batch.
...
* **endpoint**: string (Required): Endpoint of the custom authentication server. Must be an HTTPS endpoint.
...
## ClientConfig
### Properties
* **maxKeepAliveSeconds**: int: Upper bound of a client's Keep Alive, in seconds.
* **maxMessageExpirySeconds**: int: Upper bound of Message Expiry Interval, in seconds.
* **maxPacketSizeBytes**: int: Max message size for a packet in Bytes.
* **maxReceiveMaximum**: int: Upper bound of Receive Maximum that a client can request in the CONNECT packet.
* **maxSessionExpirySeconds**: int: Upper bound of Session Expiry Interval, in seconds.
...
* **key**: string (Required): The key of the dataset.
...
### Properties
* **datasets**: [DataflowBuiltInTransformationDataset](#dataflowbuiltintransformationdataset)[]: Enrich data from Broker State Store. Dataset references a key in Broker State Store.
...
* **host**: string (Required): Host of the Azure Data Explorer in the form of <cluster>.<region>.kusto.windows.net .
...
* **host**: string (Required): Host of the Azure Data Lake in the form of <account>.blob.core.windows.net .
...
* **host**: string (Required): Host of the Microsoft Fabric in the form of https://<host>.fabric.microsoft.com.
...
### Properties
* **latencyMs**: int: Batching latency in milliseconds.
...
* **keepAliveSeconds**: int: Broker KeepAlive for connection in seconds.
* **maxInflightMessages**: int: The max number of messages to keep in flight. For subscribe, this is the receive maximum. For publish, this is the maximum number of messages to send before waiting for an ack.
* **protocol**: 'Mqtt' | 'WebSockets' | string: Enable or disable websockets.
* **qos**: int: Qos for Broker connection.
...
* **healthState**: 'Available' | 'Degraded' | 'Unavailable' | 'Unknown' | string (ReadOnly): The health state of the resource.
* **instanceCount**: int: To manually scale the dataflow profile, specify the maximum number of instances you want to run.
* **provisioningState**: 'Accepted' | 'Canceled' | 'Deleting' | 'Failed' | 'Provisioning' | 'Succeeded' | 'Updating' | string (ReadOnly): The status of the last operation.
...
## Frontend
### Properties
* **replicas**: int (Required): The desired number of frontend instances (pods).
* **workers**: int: Number of logical frontend workers per instance (pod).
...
* **nodePort**: int: Kubernetes node port. Only relevant when this port is associated with a `NodePort` listener.
* **port**: int (Required): TCP port for accepting client connections.
...
## Metrics
### Properties
* **prometheusPort**: int: The prometheus port to expose the metrics.
...
* **healthState**: 'Available' | 'Degraded' | 'Unavailable' | 'Unknown' | string (ReadOnly): The health state of the resource.
* **host**: string (Required): The Container Registry endpoint hostname.
* **provisioningState**: 'Accepted' | 'Canceled' | 'Deleting' | 'Failed' | 'Provisioning' | 'Succeeded' | 'Updating' | string (ReadOnly): The status of the last operation.
...
## SelfCheck
### Properties
* **intervalSeconds**: int: The self check interval.
* **mode**: 'Disabled' | 'Enabled' | string: The toggle to enable/disable self check.
* **timeoutSeconds**: int: The timeout for self check.

## SelfTracing
### Properties
* **intervalSeconds**: int: The self tracing interval.
...
### Properties
* **length**: int: The maximum length of the queue before messages start getting dropped.
...
## Traces
### Properties
* **cacheSizeMegabytes**: int: The cache size in megabytes.
...
* **spanChannelCapacity**: int: The span channel capacity.
