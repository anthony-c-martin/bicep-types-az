...
* **tenantId**: string: The Microsoft Entra tenant ID owning these domains. Defaults to the calling user's tenant when omitted.
...
## ExternalId
### Properties
* **type**: string (Required): Identifier type. Currently `DUNS` is supported.
* **value**: string (Required): Identifier value (e.g. the DUNS number string).

## GenerateUploadTokenResponse
### Properties
* **token**: string (Required): The time-bound, principal-bound upload token
...
* **value**: string: Registration number value (e.g. the VAT / GST / EIN identifier string).
...
## SoldTo
### Properties
* **addressLine1**: string: First address line. PII — postal address.
* **addressLine2**: string: Second address line. PII — postal address.
* **addressLine3**: string: Third address line. PII — postal address.
* **city**: string: City of the address. PII — postal address.
* **companyName**: string: Company name.
* **country**: string: ISO 3166-1 alpha-2 country code of the sold-to address (e.g. `US`, `IE`, `GB`). Populated from the billing account.
* **district**: string: District or sub-region. PII — postal address.
* **email**: string: Contact email address. PII — contact.
* **firstName**: string: Contact first name. PII — contact.
* **lastName**: string: Contact last name. PII — contact.
* **middleName**: string: Contact middle name. PII — contact.
* **phoneNumber**: string: Contact phone number (free-form string; format not validated server-side). PII — contact.
* **postalCode**: string: Postal or ZIP code. PII — postal address.
* **region**: string: State, province, or region. PII — postal address.
...
## TaxId
### Properties
* **country**: string: ISO 3166-1 alpha-2 country code the tax id is registered in.
* **scope**: string: Scope of the tax id (e.g. `Federal`, `State`).
* **status**: 'invalid' | 'other' | 'valid' | string: Status of the tax id.
* **type**: string: Type of tax id (e.g. `VAT`, `GST`, `EIN`). Extensible — additional types may be returned as upstream registries evolve.
* **value**: string: Tax id value (e.g. the VAT / GST / EIN identifier string).
