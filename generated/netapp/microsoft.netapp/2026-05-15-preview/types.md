...
* **site**: string (Required): The Active Directory site the service will limit Domain Controller discovery to
* **smbServerName**: string: NetBIOS name of the SMB server. This name will be registered as a computer account in the AD and used to mount volumes
* **userName**: string: A domain user account with permission to create machine accounts
...
* **secretName**: string (Required): The name of the secret in Azure Key Vault that contains the Bind DN user password.
...
### Properties
* **certificateCommonName**: string (ReadOnly): Certificate Common Name taken from the certificate installed on the bucket server
* **certificateExpiryDate**: string (ReadOnly): The bucket server's certificate expiry date.
* **certificateObject**: string (WriteOnly): The base64-encoded contents of a PEM file, which includes both the bucket server's certificate and private key. It is generated by the end user and allows the user to access volume data in a read-only manner.
...
* **filePath**: string (Required): The file path of the Cache.
...
* **size**: int (Required): Maximum storage quota allowed for a file system in bytes. Valid values are in the range 50GiB to 1PiB. Values expressed in bytes as multiples of 1GiB.
...
* **certificateName**: string: The name of the bucket server certificate stored in the Azure Key Vault.
...
* **secretName**: string: The name of the secret stored in Azure Key Vault. The associated key pair has the following structure:
...
* **currentZone**: string (ReadOnly): Indicates the current zone of the pool. This can be changed for zoneRedundant service level pool with the changeZone action
...
* **size**: int (Required): Provisioned size of the pool (in bytes). For zoneRedundant service level pool, value must be in the range 1TiB to 16TiB or 1TiB to 128TiB for supported region. Values expressed in bytes as multiples of 1TiB till 16TiB and in multiples of 8TiB from 24TiB to 128TiB. Pool size can't be shrunk once it is created.
...
* **ruleIndex**: int: Controls the priority of the export policy rule. When connecting to the volume the rule with the lowest index that applies to the connecting client is used
...
* **filePath**: string (Required): A unique file path for the volume. Used when creating mount targets. This needs to be unique within the elastic capacity pool.
...
* **size**: int (Required): Maximum size allowed for a volume in bytes. Valid values are in the range 1GiB to 16TiB. Values expressed in bytes as multiples of 1 GiB.
...
* **certificateName**: string (Required): The name of the certificate in Azure Key Vault.
* **userAssignedIdentity**: string: The ARM resource identifier of the user assigned identity used to authenticate with key vault.

## EntraIdConfig
### Properties
* **applicationId**: string (Required): ApplicationId of the app created by customer to provide authentication and required API permissions for Microsoft Graph endpoint.
* **domain**: string (Required): Domain of the Active directory synced to Entra ID for hybrid identities.
...
* **serverNamePrefix**: string (Required): Using ServerNamePrefix, FQDN (Fully Qualified Domain Name) will be generated for SMB share, using this FQDN, SMB Share will be mounted on Entra Joined VM.
...
* **bindDN**: string: The distinguished name (DN) to bind as when performing LDAP operations.
...
* **groupDN**: string: This specifies the group DN (Distinguished Name), which overrides the base DN for group lookups.
* **ldapOverTLS**: bool: Specifies whether or not the LDAP traffic needs to be secured via TLS.
* **ldapPort**: int: Port number for LDAP communication. Default is 389 for LDAP.
* **ldapServers**: string[]: List of LDAP server IP addresses (IPv4 only) for the LDAP domain.
* **netGroupDN**: string: This specifies the netgroup DN (Distinguished Name), which overrides the base DN for netgroup lookups.
* **serverCACertificate**: string: When LDAP over SSL/TLS is enabled, the LDAP client is required to have base64 encoded ldap servers CA certificate.
* **userDN**: string: This specifies the user DN (Distinguished Name), which overrides the base DN for user lookups.
...
### Properties
* **peerAddresses**: string[] (Required): ONTAP Intercluster LIF IP addresses. One IP address per cluster node is required
...
* **usageThresholdPercentage**: int: The usageThresholdPercentage filter takes the usage threshold percentage and returns records where the usage is greater than or equal to the input value. This is an optional property.
...
* **replicationId**: string (ReadOnly): UUID v4 used to identify the replication.
...
### Properties
* **keyVaultUri**: string (Required): The Uri of KeyVault.
* **secretName**: string (Required): The name of KeyVault password secret.
...
* **usageThreshold**: int (Required): Maximum storage quota allowed for a file system in bytes. This is a soft quota used for alerting only. For regular volumes, valid values are in the range 50GiB to 100TiB.
