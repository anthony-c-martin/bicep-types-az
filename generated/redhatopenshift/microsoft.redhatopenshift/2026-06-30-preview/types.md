...
## ApiProfile
### Properties
* **authorizedCidrs**: string[]: The list of authorized IPv4 CIDR blocks allowed to access the API server. Maximum 500 entries.
...
### Properties
* **maxNodeProvisionTimeSeconds**: int: maxNodeProvisionTimeSeconds is the maximum time to wait for node provisioning before considering the
provisioning to be unsuccessful. The default is 900 seconds, or 15 minutes.
* **maxNodesTotal**: int: maxNodesTotal is the maximum allowable number of nodes for the Autoscaler scale out to be operational.
The autoscaler will not grow the cluster beyond this number. If omitted, the autoscaler will not have a maximum limit.
* **maxPodGracePeriodSeconds**: int: maxPodGracePeriod is the maximum seconds to wait for graceful pod termination before scaling down a NodePool.
...
* **message**: string (Required, ReadOnly): A human readable message indicating details about the transition.
This may be an empty string.
* **reason**: string (Required, ReadOnly): A programmatic identifier indicating the reason for the condition's last transition.
...
* **baseDomainPrefix**: string: BaseDomainPrefix is the unique name of the cluster representing the OpenShift's cluster name.
...
### Properties
* **authClientNamespace**: string (Required): The namespace of the external Auth client
...
* **name**: string (Required): The name of the external auth client
...
### Properties
* **clientId**: string (Required): External Auth client id
...
* **clients**: [ExternalAuthClientProfile](#externalauthclientprofile)[]: External Auth OIDC clients
...
## GroupClaimProfile
### Properties
* **claim**: string (Required): Claim name of the external profile
...
* **imageDigestMirrors**: [ImageDigestMirror](#imagedigestmirror)[]: imageDigestMirrors is a set of rules to allow pulling images from a
...
## ImageDigestMirror
### Properties
* **mirrors**: string[] (Required): mirrors is zero or more locations that may also contain the same images. No mirror will
...
* **source**: string (Required): source matches the repository that users refer to, e.g. in image pull specifications.
...
* **vaultName**: string (Required): vaultName is the name of the keyvault that contains the secret.
* **visibility**: 'Private' | 'Public' | string (Required): visibility of the keyvault that contains the secret.

## KmsKey
### Properties
* **name**: string (Required): name is the name of the keyvault key used for encryption/decryption.
* **version**: string (Required): version contains the version of the key to use.
...
## NodePoolAutoScaling
### Properties
* **max**: int: The maximum number of nodes in the node pool.
Validation:
...
* **min**: int: The minimum number of nodes in the node pool.
Validation:
- Minimum: 0
- Maximum: 200 (only when availabilityZone is not specified)
- No maximum when availabilityZone is specified
...
* **replicas**: int: The number of worker nodes, it cannot be used together with autoscaling.
Validation:
- Minimum: 0
- Maximum: 200 (only when availabilityZone is not specified)
- No maximum when availabilityZone is specified
...
* **sizeGiB**: int: The OS disk size in GiB. Maximum is 4095 GiB for Managed disks. For Ephemeral disks,
...
## ResourceStatus
### Properties
* **conditions**: [Condition](#condition)[] (ReadOnly): The conditions on the resource
...
* **key**: string (Required): The key of the taint
...
## TokenIssuerProfile
### Properties
* **audiences**: string[] (Required): This configures the acceptable audiences the JWT token, issued by the identity
...
## TokenRequiredClaim
### Properties
* **claim**: string (Required): Claim name for the validation profile
claim is a required field that configures the name of the required claim.
* **requiredValue**: string (Required): Required value
