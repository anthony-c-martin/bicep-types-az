...
* **content**: string (WriteOnly): Specifies the XML formatted content that is added to the unattend.xml file for
...
* **vmNamePrefix**: string: VirtualMachine prefix to be used for the virtual machines launched by Fleet. Can be used only with Launch mode.
...
* **minCapacity**: int: Minimum capacity to achieve which cannot be updated. If we will not be able to "guarantee" minimum capacity, we will reject the request in the sync path itself.
...
### Properties
* **adminPassword**: string (WriteOnly): Specifies the password of the administrator account. <br><br> **Minimum-length
...
* **customData**: string (WriteOnly): Specifies a base-64 encoded string of custom data. The base-64 encoded string
...
### Properties
* **max**: int: Maximum value. Double.MaxValue(1.7976931348623157E+308)
* **min**: int: Minimum value. default 0. Double.MinValue()

## VMAttributeMinMaxInteger
### Properties
* **max**: int: Max VMSize from CRS, Max = 4294967295 (uint.MaxValue) if not specified.
* **min**: int: Min VMSize from CRS, Min = 0 (uint.MinValue) if not specified.
...
* **rank**: int: The rank of the VM size. This is used with 'RegularPriorityAllocationStrategy.Prioritized'
...
## ZonePreference
### Properties
* **rank**: int: The rank of the zone. This is used with 'Prioritized' ZoneDistributionStrategy.
The lower the number, the higher the priority, starting with 0.
